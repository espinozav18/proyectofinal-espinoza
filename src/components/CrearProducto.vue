...
export default {
  data: () => ({
    ...
    imagenes: [
      ...
      "https://images.pexels.com/photos/3738762/pexels-photo-3738762.jpeg?h=650&w=940"
  ...
  methods: {
    async validate() {
      ...
      let imagenesBack=this.imagenes;
      if(rnd<4){
        imagenesBack.splice(parseInt(Math.random()*2),1);
      }else if(rnd>4 && rnd<7){
        imagenesBack.splice(parseInt(Math.random()*2),1);
        //imagenesBack.splice(0,1);
      }
    this.prodItem.imagenes=imagenesBack;
      if (validar) {
        await axios
          .post(
            `https://61b75f4e64e4a10017d18ae0.mockapi.io/productos/`,
            this.prodItem
          )
          .then((rpta) => {
